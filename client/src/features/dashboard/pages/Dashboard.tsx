  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
        <DashboardCard title="Street Lights" value={stats.streetLights} icon={<MdLightbulb size={26} />} color="#0F766E" />
        <DashboardCard title="Complaints" value={stats.complaints} icon={<MdAssignment size={26} />} color="#F59E0B" />
        <DashboardCard title="New / Unassigned" value={stats.newComplaints} icon={<MdNewReleases size={26} />} color="#DC2626" />
        <DashboardCard title="Completed" value={stats.completed} icon={<MdCheckCircle size={26} />} color="#16A34A" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-white rounded-xl shadow border p-4 sm:p-6">
          <div className="flex justify-between items-center mb-4 gap-2">
            <h2 className="text-base sm:text-lg font-semibold text-slate-800">Breakdown</h2>
            <select value={breakdownField} onChange={(e) => setBreakdownField(e.target.value as keyof DashboardResponse["breakdowns"])} className="border rounded-lg p-1.5 text-xs sm:text-sm max-w-[140px] sm:max-w-none">
          ...
          {entries.length === 0 ? (
            ...
            <div className="space-y-3">
              {entries.map(([value, count]) => (
                <button key={value} onClick={() => handleBreakdownClick(value)} className="w-full text-left group py-1">
                  <div className="flex justify-between text-sm mb-1 gap-2">
                    <span className="text-slate-600 group-hover:text-teal-700 truncate">
                      {breakdownField === "ward_number" ? `Ward ${value}` : value.replaceAll("_", " ")}
                    </span>
                    <span className="text-slate-500 font-medium shrink-0">{count}</span>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow border p-4 sm:p-6">
          <button onClick={() => navigate("/admin/mslvl")} className="text-base sm:text-lg font-semibold text-slate-800 hover:text-teal-700 mb-4">
            MSLVL Workload →
          </button>
          {stats.mslvlWorkload.length === 0 ? (
            ...
            <div className="space-y-3">
              {stats.mslvlWorkload.map((crew) => (
                <button key={crew.id} onClick={() => openMslvl(crew.id)} className="w-full flex justify-between items-center gap-2 border-b last:border-b-0 pb-3 last:pb-0 hover:bg-slate-50 -mx-1 px-1 py-1 rounded">
                  <span className="text-slate-700 font-medium truncate">{crew.full_name}</span>
                  <span className={`text-xs px-2.5 sm:px-3 py-1 rounded-full font-medium shrink-0 ${crew.activeJobs === 0 ? "bg-slate-100 text-slate-500" : "bg-amber-100 text-amber-700"}`}>
                    {crew.activeJobs} active
                  </span>
                </button>
              ))}
            </div>
